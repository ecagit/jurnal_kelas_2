  ...
  <q-page class="q-pa-sm">
    <q-card v-if="!showForm" flat bordered>
      <q-table
        ...
        separator="cell"
        binary-state-sort
        no-data-label="Silahkan pilih guru dan kelas/hari untuk menampilkan data"
        no-results-label="Tidak ada data untuk kriteria ini"
        class="my-zebra-table"
      >
        <template v-slot:top-left>
          <div class="row items-center q-gutter-sm">
            <div class="text-h6 q-mr-md">Data Jadwal Per Guru</div>
            <q-select
              v-model="filterKelas"
        ...
        <template v-slot:top-right>
          <div class="row items-center q-gutter-sm">
            <q-select
              ...
              style="min-width: 220px; background: #f1f5f9; border-radius: 4px"
            >
              <template v-slot:no-option>
                <q-item><q-item-section class="text-grey">Tidak ada hasil</q-item-section></q-item>
              </template>
    ...
    <q-card v-else flat bordered>
      <q-card-section class="row items-center q-pb-none">
        ...
        <div class="text-h6">{{ isEdit ? 'Edit Data Jadwal' : 'Tambah Data Jadwal Baru' }}</div>
      </q-card-section>
...
// State tambahan untuk filter guru (wajib)
const filterGuru = ref(null)

// Options dropdown
const kelasOptions = ref([])
const filterGuruOptions = ref([])
const allGuruOptions = ref([])
...
// 🔥 Default sorting multi-kolom: n_hari_id, c_jam_id, c_kelas_id
const pagination = ref({
  sortBy: 'n_hari_id,c_jam_id,c_kelas_id',
  descending: false,
  page: 1,
  rowsPerPage: 20,
  rowsNumber: 0,
...
const columns = [
  { name: 'no', label: 'NO', align: 'center', field: 'no', style: 'width: 30px' },
  { name: 'c_periode', label: 'PERIODE', align: 'left', field: 'c_periode', sortable: true },
  { name: 'n_hari_id', label: 'HARI', align: 'left', field: 'n_hari_id', sortable: true },
  { name: 'c_kelas_id', label: 'KELAS', align: 'left', field: 'c_kelas_id', sortable: true },
  { name: 'c_jam_id', label: 'JAM', align: 'left', field: 'c_jam_id', sortable: true },
  { name: 'c_guru_id', label: 'GURU', align: 'left', field: 'c_guru_id', sortable: true },
  { name: 'c_jad_str', label: 'MULAI', align: 'center', field: 'c_jad_str', sortable: true },
  { name: 'n_jad_dur', label: 'DURASI', align: 'center', field: 'n_jad_dur', sortable: true },
  { name: 'c_jad_end', label: 'JAM SELESAI', align: 'center', field: 'c_jad_end', sortable: true },
  { name: 'b_aktif', label: 'AKTIF', align: 'center', field: 'b_aktif', sortable: true },
  { name: 'b_add', label: 'JAM +', align: 'center', field: 'b_add', sortable: true },
  { name: 'c_keterangan', label: 'KETERANGAN', align: 'left', field: 'c_keterangan' },
...
const getGuruNama = (id) => {
  if (!id) return '-'
  const found = allGuruOptions.value.find((opt) => opt.value === id)
  return found ? found.label : id
}
const getJamNama = (id) => {
  if (!id) return '-'
...
const loadDropdowns = async () => {
  try {
    allGuruOptions.value = await getGuruLookup()
    filterGuruOptions.value = await auth.getGuruLookup()
    filteredGuruOptions.value = [...filterGuruOptions.value]
    ...
    if (filterGuruOptions.value.length === 1) filterGuru.value = filterGuruOptions.value[0].value
...
const filterGuruFn = (val, update) => {
  ...
  update(() => {
    const needle = val.toLowerCase()
    filteredGuruOptions.value = filterGuruOptions.value.filter(
      (v) => v.label.toLowerCase().indexOf(needle) > -1,
    )
  })
}
...
const onRequest = async (props) => {
  ...
  try {
    ...
    let filterString = `c_periode = "${activePeriodeId.value}" && c_guru_id = "${filterGuru.value}"`
    if (filterKelas.value) filterString += ` && c_kelas_id = "${filterKelas.value}"`
    if (filterHari.value) filterString += ` && n_hari_id = "${filterHari.value}"`
    ...
    if (sortBy) {
      ...
      sortString = fields.map((f) => `${prefix}${f.trim()}`).join(',')
    } else {
      sortString = '+n_hari_id,+c_jam_id,+c_kelas_id'
    }
...
const confirmGenerate = () => {
  ...
  if (!activePeriodeId.value) {
    $q.notify({ type: 'warning', message: 'Periode aktif belum terdeteksi.' })
    return
  }
