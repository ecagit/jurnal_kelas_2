...
// Dinamis label jika tabel kosong
...
const loadMuridOptions = async () => {
  try {
    ...
    // [MODIFIKASI] Sisipkan nama/ID kelas ke dalam label agar mudah dibaca di Dropdown
    muridOptions.value = murids.map((m) => {
      const kelasNama = getKelasNama(m.c_kelas_id)
      return {
        value: m.c_murid_id,
        label: `${m.c_nama} (${m.c_nis || m.c_murid_id}) - Kelas: ${kelasNama}`,
    ...
    filteredMuridOptions.value = getBaseMuridOptions()
...
const getBaseMuridOptions = () => {
  const isAdmin = pb.authStore.model?.c_default_role === 'Admin'
  let base = []

  if (filterKelas.value) {
    base = muridOptions.value.filter((m) => m.kelas_id === filterKelas.value)
  } else {
    base = isAdmin ? muridOptions.value : []
  }

  if (form.value.c_emp_id && !base.find((m) => m.value === form.value.c_emp_id)) {
    const currentSelected = muridOptions.value.find((m) => m.value === form.value.c_emp_id)
    if (currentSelected) {
      base.push(currentSelected)
    }
  }
...
const filterMuridFn = (val, update) => {
  update(() => {
    const baseOptions = getBaseMuridOptions()

    if (val === '') {
      filteredMuridOptions.value = baseOptions
    } else {
      const needle = val.toLowerCase()
      filteredMuridOptions.value = baseOptions.filter(
