...
// ============================================
// 🔥 REVISI PENDEKATAN PREVENTIF: HELPER URL 🔥
// ============================================

// 1. Fungsi mengubah HTML menjadi Relatif (Simpan ke DB)
const makeHtmlRelative = (htmlString) => {
  if (!htmlString) return ''
  // Penjelasan Regex:
  // Mencari src="http://domain:port/api/files/..." atau https://...
  // Lalu menggantinya HANYA dengan src="/api/files/..."
  // Ini menghindari masalah "hilang slash" karena kita merekonstruksi format src="..." dengan tepat.
  return htmlString.replace(/(src|href)="https?:\/\/[^/]+(\/api\/files\/[^"]+)"/g, '$1="$2"')
}

// 2. Fungsi mengubah HTML menjadi Absolut (Tampil ke Tiptap)
const makeHtmlAbsolute = (htmlString) => {
  if (!htmlString) return ''

  // Pastikan pb.baseUrl tidak memiliki trailing slash (/) di akhir
  // untuk mencegah URL menjadi double slash (misal: http://localhost:8095//api/files/...)
  const base = pb.baseUrl.replace(/\/$/, '')

  // Mencari src="/api/files/..." dan menyelipkan base URL di depannya
  return htmlString.replace(/(src|href)="(\/api\/files\/[^"]+)"/g, `$1="${base}$2"`)
}
...
const simpanData = async () => {
  try {
    ...
    const payload = {
      ...
      // 🔥 Eksekusi konversi ke relatif sebelum payload dikirim ke PB 🔥
      c_post: makeHtmlRelative(kontenHtml),
...
const bukaFormEdit = (item) => {
  ...
  // 🔥 Eksekusi konversi ke absolut sebelum masuk ke Editor Tiptap 🔥
  let kontenDariDb = item.c_post || ''
  let kontenAbsolut = makeHtmlAbsolute(kontenDariDb)
