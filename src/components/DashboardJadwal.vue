...
// Ekstrak activeRole dari auth store agar reaktif dan bisa di-watch
const { activeRole } = storeToRefs(auth)
...
const loadDashboardData = async () => {
  ...
  try {
    // REVISI: Selalu update dropdown kelas untuk memastikan daftar
    // sinkron dengan Role yang sedang aktif (tidak lagi diblokir oleh if (length === 0))
    await loadDropdowns()

    // Jika user berganti ke role yang tidak memiliki akses kelas sama sekali
    if (kelasOptions.value.length === 0) {
      jadwalRows.value = []
      return
    }

    // 1. AMBIL DATA MASTER JAM (tb_mst_jamsat)
    ...
    const dataDashboard = kelasOptions.value.map((kelas) => {
      ...
      const jadwalWajib = jadwalKelasIni.filter((j) => j.b_utama === true)
    ...
    if (error.message?.includes('autocancelled') || error.message?.includes('aborted')) {
      console.log('Request dibatalkan (auto-cancel) - normal saat pergantian role')
      return
    }
...
// Watch perubahan periode
watch(activePeriodeId, (newVal) => {
  if (newVal) loadDashboardData()
})

// TAMBAHAN: Watch perubahan peran/role
watch(activeRole, (newRole) => {
  if (newRole) loadDashboardData()
})

// Expose method agar bisa dipanggil dari parent jika perlu
defineExpose({
...
<template>
  <div class="dashboard-jadwal-wrapper">
